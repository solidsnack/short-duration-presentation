...
const YEAR: f64 = DAY * 365.2425;
// Special interval needed for rounding, when using weeks and years together,
// and when choosing whether a value can be represented with weeks and days or
// not, because a year is not an integer number of days or weeks.
const END_OF_YEAR: f64 = YEAR - (51.0 * WEEK);
...
fn div_rem_div(n: f64, greater: f64, lesser: f64) -> (f64, f64) {
...
mod tests {
    ...
    fn test_year_boundaries() {
        ...
        assert_eq!("1y51w", present(base + offset - 2.0));
        assert_eq!("2y00w", present(base + offset));

        let almost_ten = (9.0 * YEAR) + (51.0 * WEEK) + (4.0 * DAY);
        assert_eq!("9y51w", present(almost_ten));

        assert_eq!("9y51w", present(almost_ten + offset - 1.0));
        assert_eq!("0010y", present(almost_ten + offset));
    }
    ...
    fn test_transitions() {
        ...
        assert_eq!("99w6d", present(base - 1.0));
